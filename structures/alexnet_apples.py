...
def create_structure(tf, x):
    ...
    with tf.variable_scope("structure"):
        ...
        fc4 = maker.weight_variable(tf, shape=[1000, 5], name="fc4")
        bfc4 = maker.bias_variable(tf, shape=[5], name="bfc4")
        ...
        h_fc4 = tf.nn.relu(tf.matmul(h_fc3, fc4) + bfc4)

        return tf.nn.softmax(h_fc4)
